...
#+POSTID: 1472
#+title: Emacs Configuaration of Mine
#+date: 2023-07-26
#+tags: Technical Emacs Configuaration Customization Editor Opensource

As the title of this post says, I tried to present it in a formatted manner, so it can
be visually pleasing to the eyes.
...
(setq org-capture-templates
      (quote (("t" "Todo" entry (file "~/.emacs.d/OrgFiles/task.org")
              ...
              ("d" "Diary" entry (file+datetree "~/.emacs.d/OrgFiles/diary.org")
               "* %?\n%U\n" :clock-in t :clock-resume t)
              ("R" "Refile" entry (file "~/.emacs.d/OrgFiles/refile.org")
               "*  %A %^G\n%c\n%U\n" :immediate-finish t)
...
* IRC
#+BEGIN_SRC emacs-lisp
(add-to-list 'load-path "~/.emacs.d/erc-sasl.el");; IRC
...
* Persistent highlight with tooltip note
#+begin_src emacs-lisp
...
* Restrict Elfeed entry to 1 month
#+begin_src emacs-lisp
...
* Stop creating backup files and put all the backup file in one directory
#+begin_src emacs-lisp
...
* Capture selected pdf doc text as note
#+begin_src emacs-lisp
...
* Call up proced aka system process viewer
...
* xclip helps to copy and paste from clipboard
#+begin_src emacs-lisp
(xclip-mode 1)
#+end_src
...
* Convenience for M-x on the terminal
#+BEGIN_SRC emacs-lisp
...
* Language Tool
#+BEGIN_SRC emacs-lisp
(setq langtool-language-tool-jar "/home/bhaskar/LanguageTool-5.7/languagetool-commandline.jar")
(require 'langtool)

(global-set-key "\C-x4l" 'langtool-check)
...
(use-package citar
    ...
    (citar-library-paths '("~/bibliography/library/"))
    :bind
    (("C-c t o" . citar-open)
     (:map org-mode-map :package org ("C-c b" . #'org-cite-insert))))
#+END_SRC

* Systemd config file color syntax
#+BEGIN_SRC emacs-lisp
...
* Org footnotes frequently used options
#+BEGIN_SRC emacs-lisp
...
* Some specific key bind for double quote
